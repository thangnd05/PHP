...
// Thêm vào cuối file employee.php
...
function add_account($employee_id, $username, $password, $role_id): void
{
  global $conn;
  connect_db();
...
function delete_account($account_id): void
{
  global $conn;
  connect_db();
  $stmt = $conn->prepare("DELETE FROM employee_accounts WHERE account_id = ?");
